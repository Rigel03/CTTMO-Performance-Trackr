...
export async function getIndicators(activeOnly = false) {
  let query = supabase.from('performance_indicators').select('*').order('function_name').order('indicator_desc');
...
export async function updateIndicator(id, data) {
  const updateData = {};
  if (data.functionName !== undefined) updateData.function_name = data.functionName;
